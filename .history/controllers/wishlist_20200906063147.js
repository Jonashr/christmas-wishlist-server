...
const calculateTimeElapsedBetweenBirthdateAndCurrentDate = require('../helpers/dateTimeCalculationHelpers').calculateTimeElapsedBetweenBirthdateAndCurrentDate
...
wishlistRouter.post('/', async(req, res, next) => {
  ...
  try {
    ...
    // For the last entry.
    const userAge = calculateTimeElapsedBetweenBirthdateAndCurrentDate(moment(), moment(queriedUserProfile.birthdate, 'YYYY-DD-MM'))
