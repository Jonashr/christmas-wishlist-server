...
wishlistRouter.post('/', async(req, res, next) => {
  ...
  try {
    ...
    console.log('Time elapsed method', calculateTimeElapsedFromDateAToDateB(moment('2010-06-09', 'YYYY-DD-MM'), moment()))
...
const calculateTimeElapsedFromDateAToDateB = (dateA, dateB) => {
  ...
  const timeElapsed = moment(dateA, 'YYYY-DD-MM').diff(moment(dateB, 'YYYY-DD-MM'), 'years', true)
  return timeElapsed
}
